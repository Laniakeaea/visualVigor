...
export class RectangleTool {
    ...
    onKeyDown(e) {
        if (this.isDrawing && (e.key === 'Shift' || e.key === 'Alt')) {
            ...
            // We need the last mouse position, but we don't have it here easily without tracking it.
            // For now, we rely on the next mouse move or just let it be.
            // Ideally we track lastMousePoint.
        }
    }
    ...
    updatePreview(currentPoint, isSquare, isCenter) {
        ...
        let rectEl = previewLayer.firstChild;
        if (!rectEl || rectEl.tagName !== 'rect') {
            while (previewLayer.firstChild) {
                previewLayer.removeChild(previewLayer.firstChild);
            }
            rectEl = document.createElementNS("http://www.w3.org/2000/svg", "rect");
            rectEl.setAttribute('fill', 'none');
            previewLayer.appendChild(rectEl);
        }
        ...
        rectEl.setAttribute('stroke', this.getStrokeColor());
        rectEl.setAttribute('stroke-width', this.options.size);
        rectEl.setAttribute('fill', this.getFillColor());
    }
    ...
    createVectorElement(start, end, isSquare, isCenter) {
        ...
        // Don't create zero-size rects
        if (rect.width < 0.1 || rect.height < 0.1) return;

        const element = window.projectModel.addVectorElement('rect');
        if (element) {
            element.properties = {
                x: rect.x,
                y: rect.y,
