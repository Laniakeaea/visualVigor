...
export class PolygonTool {
    ...
    calculatePoints(rect, sides) {
        ...
        const points = [];
        for (let i = 0; i < n; i++) {
            // Start from top (angle 0 is usually right, so subtract PI/2)
            // But in the reference implementation:
            // ctx.moveTo(0, -ry); -> This is top.
            // angle = (i * 2 * Math.PI) / n;
            // x = sin(angle) * rx
            // y = -cos(angle) * ry
            ...
            const px = cx + Math.sin(angle) * rx;
            const py = cy - Math.cos(angle) * ry;
            points.push(`${px},${py}`);
        }
        return points.join(' ');
    }
    ...
    updatePreview(currentPoint, isSquare, isCenter) {
        ...
        let polyEl = previewLayer.firstChild;
        if (!polyEl || polyEl.tagName !== 'polygon') {
            while (previewLayer.firstChild) {
                previewLayer.removeChild(previewLayer.firstChild);
            }
            polyEl = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
            polyEl.setAttribute('fill', 'none');
            previewLayer.appendChild(polyEl);
        }

        const rect = this.calculateRect(this.startPoint, currentPoint, isSquare, isCenter);
        const pointsStr = this.calculatePoints(rect, this.options.sides);

        polyEl.setAttribute('points', pointsStr);
        polyEl.setAttribute('stroke', this.getStrokeColor());
        polyEl.setAttribute('stroke-width', this.options.size);
        polyEl.setAttribute('fill', this.getFillColor());
    }
    ...
    createVectorElement(start, end, isSquare, isCenter) {
        ...
        const properties = {
            points: this.calculatePoints(rect, this.options.sides),
            sides: this.options.sides, // Store sides for potential future editing
            x: rect.x, // Store bounds for reference
            y: rect.y,
            width: rect.width,
            height: rect.height,
        ...
        const element = window.projectModel.addVectorElement('polygon', properties);
        if (element) {
            // Properties are already set by addVectorElement
            
            // window.dispatchEvent(new CustomEvent('projectLayersChanged')); // Already dispatched by addVectorElement
