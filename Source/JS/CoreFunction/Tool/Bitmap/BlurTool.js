...
export class BlurTool {
    constructor() {
        this.id = 'toolBitmapBlur';
        this.options = {
            size: 30,
            strength: 50, // 0-100, maps to blur radius
        };
        ...
        try {
            ...
            console.error('BlurTool: WebGL not supported or glfx failed', e);
            this.fxCanvas = null;
        }
    }
    ...
    _applyBlur(ctx, pt) {
        ...
        // 2. Apply Blur to brush canvas
        // Map strength 0-100 to radius 0-10 (approx)
        const blurRadius = Math.max(0.1, this.options.strength / 10);
        
        if (this.fxCanvas) {
            try {
                const texture = this.fxCanvas.texture(this.brushCanvas);
                this.fxCanvas.draw(texture).triangleBlur(blurRadius).update();
                ...
                console.error('BlurTool: glfx error', e);
            }
        }
