...
export class DesaturateTool {
    constructor() {
        this.id = 'toolBitmapDesaturate';
        this.options = {
            size: 30,
            strength: 50, // 0-100, maps to saturation reduction
        ...
        try {
            ...
            console.error('DesaturateTool: WebGL not supported or glfx failed', e);
            this.fxCanvas = null;
        }
    }
    ...
    onPointerMove(e) {
        ...
        if (this.isDrawing) {
            const ctx = ToolUtils.validateActiveLayer().getContext('2d');
            ...
            if (dist > step) {
                ...
                for (let i = 1; i <= steps; i++) {
                    ...
                    this._applyDesaturate(ctx, {x: ix, y: iy});
                }
                this.lastPoint = pt;
            }
        }
    ...
    _applyDesaturate(ctx, pt) {
        ...
        // 2. Apply Desaturate to brush canvas
        // Map strength 0-100 to saturation -1 to 0 (approx)
        // Strength 100 => Saturation -1 (Fully desaturated)
        // Strength 0 => Saturation 0 (No change)
        const saturation = -(this.options.strength / 100);
        
        if (this.fxCanvas) {
            try {
                const texture = this.fxCanvas.texture(this.brushCanvas);
                this.fxCanvas.draw(texture).hueSaturation(0, saturation).update();
                ...
                console.error('DesaturateTool: glfx error', e);
            }
        }

        // 3. Mask it to a circle (Soft edge?)
        // To make it soft, we can use a radial gradient for the mask
